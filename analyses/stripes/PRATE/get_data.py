...
# Download the 20CRv3 monthly-mean PRATE.
...
base_url="https://portal.nersc.gov/archive/home/projects/incite11/www/"+\
         "20C_Reanalysis_version_3/everymember_anal_netcdf/mnmean/PRATE"

local_dir="%s/20CR/version_3/monthly_means" % os.getenv('SCRATCH')

for year in range(1806,2016):
    tf = "%s/%04d/PRATE.%04d.mnmean_mem039.nc" % (local_dir,year,year)
    if os.path.isfile(tf): continue
    remote = "%s/PRATE_%04d_mnmean.tar" % (base_url,year)
    local = "%s/PRATE_%04d_mnmean.tar" % (local_dir,year)
