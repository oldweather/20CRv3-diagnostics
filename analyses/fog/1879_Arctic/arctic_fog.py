...
parser.add_argument("--opdir", help="Directory for output files",
                    default="%s/images/20CRv3_arctic_fog" % \
                                           os.getenv('SCRATCH'),
...
# Sea-ice, means only available, don't bother with time interpolation
prevt=datetime.datetime(args.year,args.month,args.day,
                        int(args.hour)-int(args.hour)%6)
icec=iris.load_cube('%s/20CR/version_3/%04d/ICEC.%04d_v451.nc' % (
                      os.getenv('SCRATCH'),prevt.year,prevt.year),
                       iris.Constraint(time=iris.time.PartialDateTime(year=prevt.year,
                                                                      month=prevt.month,
                                                                      day=prevt.day,
                                                                      hour=prevt.hour)))
coord_s=iris.coord_systems.GeogCS(iris.fileformats.pp.EARTH_RADIUS)
...
lats = icec.coord('latitude').points
lons = icec.coord('longitude').points
...
fog_pc=plot_cube(0.05,-180/args.zoom,180/args.zoom,
                         -90/args.zoom,90/args.zoom)   
prmsl   = prmsl.regrid(fog_pc,iris.analysis.Linear())
prevcsd = prevcsd.regrid(fog_pc,iris.analysis.Linear())
...
# Overplot the continents in foggy areas
mask_img = ax.pcolorfast(lons, lats, mask.data*prmsl.data,
                         cmap=matplotlib.colors.ListedColormap(
                                ((0.4,0.4,0.4,0),
                                 (0.4,0.4,0.4,1))),
                         vmin=0,
