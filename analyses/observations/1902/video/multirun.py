...
def is_done(year,month,day,hour):
    op_file_name=("%s/images/v3_obs/1902/" +
                  "Observations_%04d%02d%02d%02d%02d.png") % (
                            os.getenv('SCRATCH'),
...
start_day=datetime.datetime(1902,  8,  1,  0)
end_day  =datetime.datetime(1902,  8, 31, 23)
...
while current_day<=end_day:
    for fraction in (0,.5):
        if is_done(current_day.year,current_day.month,
            ...
            continue
        cmd=("./obs_only.py --year=%d --month=%d" +
            " --day=%d --hour=%f \n") % (
