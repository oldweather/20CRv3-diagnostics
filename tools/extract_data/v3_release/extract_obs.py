...
parser.add_argument("--startyear", help="Ensda start year", type=int, required=True)
...
while current_day.month == args.month:
    ...
    for hour in range(0, 24, 6):
        ...
        for o_file in ("psobs.txt", "psobs_prior.txt", "psobs_posterior.txt"):
            if not os.path.exists("%s/%s" % (obs_dir_name, o_file)):
                raise Exception("Missing data %s/%s" % (obs_dir_name, o_file))
            copyfile(
                "%s/%s" % (obs_dir_name, o_file),
